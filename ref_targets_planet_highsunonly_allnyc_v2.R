...
# Prep input stuff
num_bands <- 4 # 4 or 8
path_planet_mosaics <- "/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_4b_highsunonly"

# to write out
output_prefix <- "all_nyc_4b_"
output_target_refl_dir <- "/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_4b_highsunonly_cal" # no trailing "/"
#...
...
test_date <- all_target_df[which(all_target_df$date == ymd("20200706")),]
...
ggplot(ref_test_merged, aes(x = mean.nir.x, y = mean.nir.y)) + 
  ...
  labs(x = "Test: 2020-07-09 NIR", y = "Ref: 2022-07-10 NIR") +
  theme_bw()
...
#for (img_ind in 1:10){
  ...
  if (nrow(test_date) < 2){
    output_empline$img_file[output_rowind:(output_rowind + 3)] <- img_list[img_ind]
    output_empline$band[output_rowind:(output_rowind + 3)] <- band_list
    output_empline[output_rowind:(output_rowind + 3), c("date", "band", "n", "slope", "intercept", "r2", "pvalue")] <- NA
...
output_empline_f <- output_empline_gtr2min[which(output_empline_gtr2min$n_kept_bands == 4),]
