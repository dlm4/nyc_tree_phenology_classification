...
i <- which(tile_polys$LAS_ID == "987202")
...
#for (i in 101:1702){
  ...
  tnc_gdb_polys_reproj_centroid_sub <- st_intersection(tnc_gdb_polys_reproj_centroid, rast_tile_extent)
    ...
    # skip buffering for this single tile visualization
    ...
    pts_classes <- c(LASLOWVEGETATION, LASMEDIUMVEGETATION, LASHIGHVEGETATION, LASGROUND, LASBUILDING) # add ground and building classes
    ...
    # remove ground for veg height, includes buildings too with previous filter
    las_veg_height <- las - terrain_raster_sub
    #rm(las)
    
    #rglwidget()
    
    # plot the height here
    lidR::plot(las_veg_height, color = "Z")
    
    plot3d(las_veg_height@data$X, las_veg_height@data$Y, las_veg_height@data$Z)
