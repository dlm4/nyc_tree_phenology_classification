...
out_path <- "/Volumes/NYC_geo/tree_classification/1timepoint_test_outputs/"
name_prefix <- "" # if any name modifications are desired appended to beginning of each output, make to sure include trailing "_"
...
col_list_sub <- col_list[grep("nd_nir_red_zscaled_mean", col_list)]
tp_list <- strsplit(col_list_sub, "_") %>% lapply('[[', 7) %>% unlist() %>% unique()

for (tp in tp_list){
  #yr <- 2024
  col_list_sub <- col_list[grep(tp, col_list)]
  ...
  # output xgb model
  setwd(out_path)
  xgb_outname <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_only.model")
  ...
  xgb_imp_df <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_importance.csv")
  write.csv(xgb_imp, xgb_imp_df, row.names = FALSE)
  
  ggplot(xgb_imp[1:25,]) +
    geom_col(aes(x = Gain, y = fct_reorder(Feature, Gain))) +
    labs(x = "Gain", y = "Variable", title = "Top 25 most important variables")
  xgb_imp_fig <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_importance.jpg")
  ...
  xgb_cm <- confusionMatrix(factor(test_prediction$max_prob),
                            factor(test_prediction$label),
                            mode = "everything")
  xgb_cm_out <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_confmat.rds")
  saveRDS(xgb_cm, xgb_cm_out)
  
  # save test prediction for referencing
  test_prediction_path <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_predictions.csv")
  ...
  test_prediction_accuracies <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_accuracies.csv")
  ...
  ggplot(cm_results) +
    ...
    theme_bw()
  xgb_acc_fig <- paste0(name_prefix, "xgb_", tp, "_", ncols, "cols_1timepoint_acc.jpg")
