...
getCorYear <- function(l_pheno_df, colname){
  return(ymd(paste(l_pheno_df$flag, month(l_pheno_df[[colname]]), day(l_pheno_df[[colname]]), sep = "-")))
}
...
calcTreePheno <- function(i, idrange, set_num, agg_method, tree_df_all, n_year, output_dir){
  ...
  write.csv(all_pheno_output, 
            paste0(output_dir, "/trees_pheno_output_objset", # output path dir with no trailing "/"
...
# For testing
  ...
  # n_year <- 8
  # start_time <- Sys.time()
  # calcTreePheno(i, idrange, set_num, agg_method, tree_df_all, n_year, output_dir)
  # stop_time <- Sys.time()
