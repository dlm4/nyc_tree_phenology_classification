...
#test_rast <- rast("/Volumes/NYC_geo/Planet/tests/bronx_daily_stack_maxcloud10_cal/bronx_planet_composite_4band_20220710_bronxcal20220710ref.tif")
#test_rast <- rast("/Volumes/NYC_geo/Planet/tests/bronx_daily_stack_clear_cal/bronx_planet_composite_4band_20220710_bronxcal20220710ref.tif")
#test_rast <- rast("/Volumes/NYC_geo/Planet/tests/bronx_daily_stack_highsunonly_cal/bronx_planet_composite_4band_20220710_bronxcal20220710ref.tif")
test_rast <- rast("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/nyc_planet_composite_4band_20220710_nyccal20220710ref.tif")

plotRGB(test_rast, r = 4, g = 3, b = 2, stretch = "lin")
#plotRGB(test_rast, r = 3, g = 2, b = 1, stretch = "lin")
#plotRGB(test_rast, r = 3, g = 4, b = 1, stretch = "lin")
...
# subset area to Bronx in Planet projection
...
# Tree extraction for entire city is almost too big to do with single files in multicore, so break it up into more manageable pieces (4 or 5)

# change to just centroids - for points
tnc_gdb_points_reproj <- st_centroid(tnc_gdb_polys_reproj)

# HERE WE GO
setwd("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal")
tif_file_list <- list.files(pattern = glob2rx("nyc_planet_composite_4band*nyccal20220710ref*"))
tif_file_list_datestrings <- strsplit(tif_file_list, "[.]") %>% lapply('[[', 1) %>% stringr::str_sub(start = -26, end = -19) # had to change the range of the str_sub(), nyc is two characters shorter than bronx
...
extractTrees <- function(i, sel_tif_images, tree_poly_sf){
  ...
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_mean/nyc_trees_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_mean.csv"), row.names = FALSE)
...
extractTreesByObjectIDRange <- function(i, sel_tif_images, tree_poly_sf, set_num){
  ...
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_mean/nyc_trees_objset", as.character(set_num), "_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_mean.csv"), row.names = FALSE)
  tmpFiles(current=TRUE, orphan=FALSE, old=FALSE, remove=TRUE)
  rm(sel_tif_rast)
  rm(extracted_trees)
  rm(extracted_trees2)
...
extractTreesNDVI75 <- function(i, sel_tif_images, tree_poly_sf){
  ...
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_ndvi75/nyc_trees_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_ndvi75.csv"), row.names = FALSE)
  tmpFiles(current=TRUE, orphan=FALSE, old=FALSE, remove=TRUE)
...
extractTreesNDVI75ByObjectIDRange <- function(i, sel_tif_images, tree_poly_sf, set_num){
  ...
  extracted_trees2 <- extracted_trees[which(complete.cases(extracted_trees)),] # screen out polygons that have NAs for reducing the size of the output files
  extracted_trees2$q75 <- round(extracted_trees2$q75*10000, digits = 0) # multiply by 10000 and round the values
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_ndvi75/nyc_trees_objset", as.character(set_num), "_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_ndvi75.csv"), row.names = FALSE)
  tmpFiles(current=TRUE, orphan=FALSE, old=FALSE, remove=TRUE)
  rm(sel_tif_rast_ndvi)
  rm(sel_tif_rast)
  gc()
...
extractTreesPoint <- function(i, sel_tif_images, tree_point_sf){
  ...
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_point/nyc_trees_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_point.csv"), row.names = FALSE)
  tmpFiles(current=TRUE, orphan=FALSE, old=FALSE, remove=TRUE)
}
...
extractTreesPointByObjectIDRange <- function(i, sel_tif_images, tree_point_sf, set_num){
  ...
  write.csv(extracted_trees2, paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_highsunonly_cal/tree_outputs_point/nyc_trees_objset", as.character(set_num), "_", as.character(sel_tif_images_datestrings[i]), "_nyccal20220710ref_point.csv"), row.names = FALSE)
  tmpFiles(current=TRUE, orphan=FALSE, old=FALSE, remove=TRUE)
}

# Need to start a multisession here
plan(multisession, workers = 10) # can increase this again, seems ok for mean calculation when full set is cut up
...
# Set up ranges for rows to loop over
...
# NDVI 75
start_time <- Sys.time()
#future_lapply(1:length(sel_tif_images), FUN = extractTreesNDVI75, sel_tif_images, tnc_gdb_polys_reproj_boro, future.seed = TRUE)
