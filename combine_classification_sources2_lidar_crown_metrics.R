...
df_cls_agg_mean_lidar_2 <- merge(df_cls_agg_mean_lidar, tree_crown_metrics, by.x = "Object_ID", by.y = "Poly_ID")
# this is slightly smaller than df_cls_agg_mean_lidar, loses 836 tree polygons. Some lidar crown metrics didn't work then, but that's ok for now


# Add in SOS and EOS variables
# Could drop EOS from the model, it's not useful and might make things harder to explain
...
df_cls_agg_mean_lidar_pheno <- merge(df_cls_agg_mean_lidar_2, pheno_vars_wide_sub, by.x = "Object_ID", by.y = "Poly_ID") #subsetting for complete is built-in on the merge
...
# this is the pre-cooked input!

# # comparing crown metrics height vs TNC UVM height
# # ft
# ggplot(df_cls_agg_mean_lidar_pheno, aes(x = height_max_ft, y = Height)) +
#   geom_point() +
#   coord_equal() +
#   geom_abline(slope = 1, intercept = 0, color = "blue") +
#   labs(x = "Max Height (ft) from us", y = "Max Height (ft) from TNC+UVM") +
#   geom_smooth(method = "lm", color = "red", linetype = "dashed") +
#   theme_bw()
# 
# x <- df_cls_agg_mean_lidar_pheno$height_max_ft
# y <- df_cls_agg_mean_lidar_pheno$Height
# summary(lm(y~x)) # y = 0.915*x + 2.319, R2 = 0.826, p < 2.2e-16
# 
# # m
# ggplot(df_cls_agg_mean_lidar_pheno, aes(x = height_max_ft/3.28, y = Height/3.28)) +
#   geom_point() +
#   coord_equal() +
#   geom_abline(slope = 1, intercept = 0, color = "blue") +
#   labs(x = "Max Height (m) from us", y = "Max Height (m) from TNC+UVM") +
#   geom_smooth(method = "lm", color = "red", linetype = "dashed") +
#   theme_bw()
# 
# x <- df_cls_agg_mean_lidar_pheno$height_max_ft/3.28
# y <- df_cls_agg_mean_lidar_pheno$Height/3.28
# summary(lm(y~x)) # y = 0.915*x + 0.707, R2 = 0.826, p < 2.2e-16

# Drop TNC polygon info (for now)
df_cls_agg_mean_lidar_pheno <- df_cls_agg_mean_lidar_pheno %>% select(!c("Height", "Radius", "SHAPE_Length", "SHAPE_Area")) # drop these columns from 

# drop any trees that have inf or nan, they don't work with XGB (but ranger is cool with them)
# https://stackoverflow.com/questions/36590230/how-to-remove-rows-with-inf-from-a-dataframe-in-r
df_cls_agg_mean_lidar_pheno_filt <- df_cls_agg_mean_lidar_pheno %>% 
  filter_if(~is.numeric(.), all_vars(!is.infinite(.))) %>% 
  filter_if(~is.numeric(.), all_vars(!is.nan(.)))
...
df_cls_agg_mean_cc <- na.omit(df_cls_agg_mean_lidar_pheno_filt) # using filtered version to get rid of infinite values
